...
impl egui_field_editor::EguiInspect for MyApp {
	fn inspect_with_custom_id(&mut self, _parent_id: egui::Id, label: &str, _tooltip: &str, label_ratio: f32, read_only: bool, ui: &mut egui::Ui) {
		...
		let mut add_content=|ui:&mut egui::Ui| {
			...
			egui_field_editor::add_combobox(&mut self.an_index, "Combobox", "", label_ratio, read_only, &["Choice 1".to_string(),"Choice 2".to_string(),"Choice 3".to_string()], ui);
			egui_field_editor::add_string_convertible(&mut self.an_ipv4, "IPv4", "", label_ratio, false, ui);
		};
		if !label.is_empty() {
